...
var myCtrlClass = function($scope) {
  $scope.vm = {
		sb : {
			name : "Jason Stantham",
			gender : "male",
...
.controller('ezController', myCtrlClass)

.directive('hgNamecard', function() {
  return {
    restrict: 'E',
    replace: true,
    template: '<div><button ng-click="vm.shuffle();">shuffle</button><div>name : {{vm.sb.name}}</div><div>gender : {{vm.sb.gender}}</div><div>age : {{vm.sb.age}}</div><div>career : {{vm.sb.career}}</div><div><img ng-src="{{vm.sb.photo}}"></div></div>'
  }
})
